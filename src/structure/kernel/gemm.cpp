...
void GemmConstantWeightsBiasKernel::Run(mlir::OpBuilder &builder,
  ...
  if (weights_raw_type == Type::FLOAT32) {
    std::vector<float32_t> weights_data(weights_ref.begin(), weights_ref.end());
    weights_elements = mlir::DenseElementsAttr::get(
        weights_shaped_type, llvm::ArrayRef<float32_t>(weights_data));
  } else {
#ifdef DEBUG
  ...
  if (bias_raw_type == Type::FLOAT32) {
    std::vector<float32_t> bias_data(bias_ref.begin(), bias_ref.end());
    bias_elements = mlir::DenseElementsAttr::get(
        bias_shaped_type, llvm::ArrayRef<float32_t>(bias_data));
  } else {
#ifdef DEBUG
    throw UnreachableException();
#else
