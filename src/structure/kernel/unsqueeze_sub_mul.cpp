...
void UnsqueezeSubLhsScalarMulRhsScalarKernel::Run(
    mlir::OpBuilder &builder, llvm::ArrayRef<int64_t> unsqueeze_axes,
...
// TODO: Only support for fp32 currently
#ifdef DEBUG
  assert(sub_type == Type::FLOAT32);
  assert(mul_type == Type::FLOAT32);
#endif
  ...
  assert(input_rank + unsqueeze_axes.size() == output_rank);
#endif
  llvm::SmallVector<mlir::AffineExpr> exprs;
  for (size_t i = 0; i < output_rank; ++i) {
    bool is_axis = false;
    for (int64_t axis : unsqueeze_axes) {
      if (i == axis) {
        is_axis = true;
        break;
      }
  ...
  builder.create<mlir::linalg::GenericOp>(
      ...
      [&](mlir::OpBuilder &b, mlir::Location loc, mlir::ValueRange inputs) {
        ...
        mlir::Value sub = b.create<mlir::arith::ConstantOp>(
            loc, builder.getF32FloatAttr(sub_val));
        mlir::Value mul = b.create<mlir::arith::ConstantOp>(
            loc, builder.getF32FloatAttr(mul_val));
